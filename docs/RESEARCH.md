...
4. **Age gate + 18+ rating.**
5. **IAP** for digital access (Guideline 3.1.1).
6. If anything is generated: treat it like UGC (filter, report, contact, you are responsible — 1.2 / 4.7).
...
**Go-to-market for review:** ship a listening sanctuary with authored sessions, heat levels, aftercare, and a Still room. Personalization is parameter assembly plus (later) server-side script adaptation — not an open spicy chat in v1.

## Vendor reality (do not be naive)

| Layer | Who | Adult fictional OK? | Use |
| --- | --- | --- | --- |
| Script LLM | **xAI Grok** | Closest fit. AUP bans CSAM, real-person pornographic likeness, non-consensual imagery. Fictional adult text is the founder’s already-validated path. | Server-side adaptation only. Never on device. |
| Script LLM | OpenAI / Anthropic | Generally **no** explicit erotic. | Do not use for Want. |
| TTS | **ElevenLabs** | Main prohibited-use policy (Sept 2025) does **not** blanket-ban adult erotica; Agents/BYO-LLM **does** ban “any sexual content”; Music bans adult-entertainment industry. Gray. | Candidate for Still and literary Want after legal review. Have a fallback. |
| TTS | Cartesia / PlayHT / many others | Often restrict NSFW. | Check TOS per vendor before a key is spent. |
...
- High-end TTS: on the order of $0.10–$0.80 per session depending on vendor and model.
- Cache the audio. Most nights will replay a loved session, not generate a new one.
